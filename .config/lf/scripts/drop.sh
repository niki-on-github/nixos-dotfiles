...
command -v dragon-drop >/dev/null ||( notify-send "Error" "Please install \"dragon-drop\"" && exit 1 )
...
filesDrop() {
    IFS=$'\n'
    for src in $(dragon-drop -t -x -p); do
