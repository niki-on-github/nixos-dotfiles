...
toggle_terminal() {
    if tmux lsp -F "#{pane_start_command}" | grep -q "$TERMINAL_TITLE" ; then
        tmux killp -t $(tmux lsp -F "#{pane_id} #{pane_start_command}" | grep "$TERMINAL_TITLE" | awk '{print $1}')
        tmux select-layout even-horizontal
    else
        cd "$(current_path)"
        if [ "$TERMINAL_POSITION" = 'bottom' ]; then
            tmux split-window -v -p 30 -f 'printf "\033]2;%s\033\\" "'$TERMINAL_TITLE'" && exec zsh'
        elif [ "$TERMINAL_POSITION" = 'right' ]; then
            tmux split-window -h -f 'printf "\033]2;%s\033\\" "'$TERMINAL_TITLE'" && exec zsh'
            tmux select-layout even-horizontal
        elif [ "$TERMINAL_POSITION" = 'left' ]; then
            tmux split-window -h -b -f 'printf "\033]2;%s\033\\" "'$TERMINAL_TITLE'" && exec zsh'
            tmux select-layout even-horizontal
        fi

        lfTerminalPaneNumber=$(tmux lsp -F "#{pane_id} #{pane_start_command}" | grep "$TERMINAL_TITLE" | awk '{print $1}')
